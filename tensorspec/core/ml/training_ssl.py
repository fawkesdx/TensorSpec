...
from tensorspec.core.ml.models import (
    SimpleCAE,
    SimCLRModel,
    ContrastiveDataset,
    NTXentLoss,
    BetaVAE,
    CNNMaskedAutoencoder,
    MoCoModel,
    BYOLModel,
    ViTMAE,
    SwAVModel,
...
ModelChangedFn = Optional[Callable[[str], None]]
...
def run_ssl_train(
    ...
    on_model_changed: ModelChangedFn = None,
) -> dict:
    device = torch.device('mps' if torch.backends.mps.is_available() else 'cpu')
    dim_E, dim_A, nY, nX = data_array.shape
    n_samples = nY * nX

    flat_data = data_array.transpose(2, 3, 0, 1).reshape(n_samples, dim_A, dim_E)
    ...
    flat_orig = data_array.transpose(2, 3, 0, 1).reshape(n_samples, 1, dim_A, dim_E)
    ...
    for model_name in selected_models:
        ...
        if model_name == "Autoencoder":
            ...
            for epoch in range(epochs):
                ...
                if on_progress:
                    on_progress(epoch + 1, total_loss / len(loader))
            model.eval()
            ...
            optimizer, criterion = torch.optim.Adam(model.parameters(), lr=lr, weight_decay=1e-4), NTXentLoss(0.5)
            for epoch in range(epochs):
                total_loss = 0
                model.train()
                for x_i, x_j in loader:
                    ...
                    total_loss += loss.item()
                if on_progress:
                    on_progress(epoch + 1, total_loss / max(1, len(loader)))
            model.eval()
            ...
            optimizer, beta = torch.optim.Adam(model.parameters(), lr=lr), 4.0
            for epoch in range(epochs):
                total_loss = 0
                model.train()
                for batch in loader:
            ...
            optimizer = torch.optim.Adam(list(model.online_encoder.parameters()) +
                                         ...
                                         list(model.online_predictor.parameters()), lr=lr)
