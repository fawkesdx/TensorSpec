...
def run_active_learning(x_arr, y_arr, labels_2d, algo, on_progress=None):
    if on_progress:
        on_progress(10, "Prepping coordinates...")
    ...
    x_margin = (x_arr.max() - x_arr.min()) * 0.2
    y_margin = (y_arr.max() - y_arr.min()) * 0.2
    new_x = np.linspace(x_arr.min() - x_margin, x_arr.max() + x_margin, int(len(x_arr) * 1.4))
    new_y = np.linspace(y_arr.min() - y_margin, y_arr.max() + y_margin, int(len(y_arr) * 1.4))
    New_X_grid, New_Y_grid = np.meshgrid(new_x, new_y)
    new_coords = np.column_stack((New_X_grid.flatten(), New_Y_grid.flatten()))
    bounds = [x_arr.min(), x_arr.max(), y_arr.min(), y_arr.max()]
    ...
    if "CPU" in algo:
        ...
        if "Gaussian Process" in algo:
            ...
            if on_progress:
                on_progress(30, "Training Gaussian Process (Locked to 1 Core for macOS Safety)...")
            ...
            if on_progress:
                on_progress(30, "Training Random Forest (Using All CPU Cores)...")
            model = RandomForestClassifier(n_estimators=100, n_jobs=-1, random_state=42)

        model.fit(X_train, y_train)

        if on_progress:
            on_progress(60, "Predicting Grid & Uncertainties...")
        chunk_size = 2000
        ...
        for i in range(0, len(norm_new_coords), chunk_size):
            ...
            if on_progress:
                on_progress(60 + int(35 * (end_idx / len(norm_new_coords))), "Predicting...")
        ...
        if "Bayesian Network" in algo:
            model = SimpleNet(use_dropout=True).to(device)
            optimizer, criterion = optim.Adam(model.parameters(), lr=0.01), nn.CrossEntropyLoss()
            if on_progress:
                on_progress(30, "Training Bayesian Network (GPU)...")
            model.train()
            for epoch in range(300):
                optimizer.zero_grad()
                loss = criterion(model(X_ten), Y_ten)
                loss.backward(); optimizer.step()
                if epoch % 50 == 0 and on_progress:
                    on_progress(30 + int(30 * (epoch / 300)), "Training...")

            if on_progress:
                on_progress(60, "Monte Carlo Sampling...")
            mc_samples = 50
            ...
            for m_idx, model in enumerate(models):
                ...
                for epoch in range(250):
                    ...
                    loss.backward(); optimizer.step()
                if on_progress:
                    on_progress(30 + int(30 * ((m_idx + 1) / num_models)), f"Trained Model {m_idx + 1}/{num_models}...")

            if on_progress:
                on_progress(60, "Ensemble Voting...")
        ...
        elif "Evidential" in algo:
            model = SimpleNet(use_dropout=False).to(device)
            optimizer = optim.Adam(model.parameters(), lr=0.01)
            if on_progress:
                on_progress(30, "Training Evidential Deep Learning Model (GPU)...")
            ...
            if on_progress:
                on_progress(60, "Calculating Dirichlet Uncertainty...")
            model.eval()
            with torch.no_grad():
    ...
    return pred_map, uncert_map, new_x, new_y, bounds


def run_simulate_al(x_arr, y_arr, labels_2d, algo, measured_mask, on_progress=None):
    if on_progress:
        on_progress(10, "Prepping simulation grid...")
    X_grid, Y_grid = np.meshgrid(x_arr, y_arr)
    coords = np.column_stack((X_grid.flatten(), Y_grid.flatten()))
    labels = labels_2d.flatten()
    ...
    coords_valid = coords[valid_mask]
    labels_valid = labels[valid_mask]
    measured_valid = measured_mask[valid_mask]
    ...
    if len(unique_train_labels) < 2:
        if on_progress:
            on_progress(100, "Need more classes to train. Picking random point...")
        ...
        return np.zeros(X_grid.shape), np.zeros(X_grid.shape), int(next_idx)
    ...
    if on_progress:
        on_progress(30, f"Training {algo}...")
    ...
    if "CPU" in algo:
        ...
        if on_progress:
            on_progress(70, "Predicting Full Grid...")
        ...
        if "Bayesian" in algo:
            ...
            if on_progress:
                on_progress(70, "Monte Carlo Sampling...")
            ...
            if on_progress:
                on_progress(70, "Calculating Dirichlet Uncertainty...")
            model.eval()
            with torch.no_grad():
            ...
            if on_progress:
                on_progress(70, "Ensemble Voting...")
    ...
    if on_progress:
        on_progress(100, "Done!")
    return pred_map_flat.reshape(X_grid.shape), uncert_map_flat.reshape(X_grid.shape), int(next_idx)
