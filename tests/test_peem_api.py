...
class TestPeemApi(unittest.TestCase):
    ...
    def test_drift_raw_writes_processed_3d(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertFalse(meta.json()["processed_is_paired"])
            self.assertIsNone(meta.json()["n_pairs"])
            self.assertEqual(meta.json()["n_processed_frames"], 2)
            self.assertEqual(meta.json()["processed_shape"], [2, 32, 32])
