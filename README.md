...
* **Suite Panels:** Crystal, DFT, and ARPES suites are live end-to-end (HTML → API → `core/`). PEEM load+view supports TIF stacks/folder sequences, companion CSV metadata, interactive frame/contrast controls, and CP/CM or LH/LV pair stacking into `/processed`; PEEM analysis and XAS / Transport / ML engines remain pending.
