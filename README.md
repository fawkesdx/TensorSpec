...
* **Suite Panels:** Crystal, DFT, and ARPES suites are live end-to-end (HTML → API → `core/`). PEEM load+view supports TIF stacks/folder sequences, companion CSV metadata, interactive frame/contrast controls, and CP/CM or LH/LV pair stacking into `/processed`, ROI NCC drift correction (writes drift-corrected stacks to `/processed`), and separate channels under `/processed/{tag}` (CP/CM or LH/LV split from paired stack). PEEM **background subtraction** — linear pre-edge or **two-step** (separate pre+post linear fits connected at midpoint; method picker in suite UI; picture-wide or ROI mean spectrum, preview/apply, `/analysis/background` + `/processed/bg`, draggable windows with light ensemble) and **XMCD sum rule** (picture-wide or ROI mean spectrum on CP/CM or LH/LV pairs, optional I0 normalization when CSV metadata aligns, Thole/Carra p/q/r integrals → orbital and spin+dipole moments, preview/apply to `/analysis/sumrule`, draggable L3/L2/r windows with dual ensemble uncertainty) are live; pixel-to-pixel sum rule and other PEEM domain analysis plus XAS / Transport / ML engines remain pending.
