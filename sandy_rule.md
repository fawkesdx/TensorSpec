...
│   │   └── peem_loaders.py   # TIF stack & sequence folder loaders[cite: 2]
│   ├── dft_engine.py         # Bulk band structure, tight binding, k.p, DFT (PBE/GGA/HSE), Green function[cite: 2]
│   ├── arpes_engine.py       # Momentum space conversion, EDC/MDC integration, peak fitting[cite: 1, 2]
│   ├── peem_engine.py        # Drift correction, CP/CM & LH/LV separation, sum rule analysis[cite: 2]
│   ├── xas_engine.py         # XAS/XMCD background subtraction and normalization[cite: 1, 2]
│   ├── transport_engine.py   # Transport analysis (magnetoresistance, Hall curves, R-T scaling)[cite: 2]
│   └── ml_engine.py          # Domain clustering, PCA/NMF dimensionality reduction, image analysis[cite: 1]
├── plotting/
│   ├── __init__.py
...
└── gui/
    ...
    └── suites/               # The 6 independent roadmap suites + ML integration
        ├── __init__.py
        ├── crystal_suite.py  # 1. Crystal Viewer: CIF loader, supercells, CDW, stack/twist, BZ[cite: 2]
        ├── dft_suite.py      # 2. DFT Suite: Band structures, slabs, Green's function surface setup[cite: 2]
        ├── arpes_suite.py    # 3. ARPES Suite: Multi-motor dispersion viewer, linked crosshairs, EDC/MDC[cite: 2]
        ├── peem_suite.py     # 4. PEEM Suite: Stack alignment, drift correction, sum rules[cite: 2]
        ├── xas_suite.py      # 5. XAS Suite: 1D spectral plotting and field/energy normalization[cite: 1, 2]
        ├── transport_suite.py# 6. Transport Suite: Curves, transport parameters, and magneto-transport[cite: 2]
        └── ml_suite.py       # ML Suite: Hyperspectral clustering and PCA/NMF decomposition[cite: 1]